...
export default function CostTrip({distance, unicFuel}){
    ...
    return (
        <div>
            ...
            {<>
            <select className={total? "valid":"error"} onChange={(e)=>setL( {...l, int: parseInt(e.target.value)} )}>
                {numbers.slice(0,20).map((n, i)=><option key={i}>{n}</option>)}
            </select>
            <select className={total? "valid":"error"} onChange={(e)=>setL( {...l, dec: parseInt( e.target.value.slice(1))*0.1} )}>
            ...
            {distance?<><br></br><br></br></>:<p className="leyenda-error">Inserte Origen, Destino y calcule la ruta.</p>}
